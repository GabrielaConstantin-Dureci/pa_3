...
`{r}
bonus_data<-select(my_data,-id, - item) %>% 
  group_by(vowel, language) %>% 
  summarize(f1_20=mean(f1_20), f1_35=mean(f1_35), f1_50=mean(f1_50),
            ...
            f2_65=mean(f2_65), f2_80=mean(f2_80), tl=mean(tl)) %>% 
...
bonus<-bonus_data %>%
  ...
  bonus_data %>% 
  ggplot(., aes( x = tl, y = f1_20, color=vowel, shape=language)) + 
  geom_point(alpha = 0.2) + 
  geom_text(data = bonus_data, aes(label = vowel), size = 5) + 
    scale_y_reverse() + 
  scale_x_reverse()
`
